# Primitive-triple filter — candidate confirmatory specification v0.5
...
confirmatory observations. v0.3 and v0.4 failed independent review before
freeze. No confirmatory observations exist.
...
JSON, source URL and signature. The v2 response contains `round` and
`signature` but no `randomness`: derive the `randomness_hex` stored in the gate
record locally as lowercase `SHA-256(signature_bytes).hexdigest()`. An
...
beacon is unavailable, do not run or choose a replacement round; if the frozen
window closes, record ABANDONED as specified in C4.

Seed construction is byte exact:

```text
SHA256( ASCII("pythagorean-filter-v0.5") || 0x00 ||
...
discarded timings. Earlier seeds are not used for v0.5 fixtures.
...
fields are recorded as null rather than guessed. The successful JSON also
contains host-clock `started_at_utc` and `finished_at_utc` for cross-checking;
the external ledger remains authoritative for process launch, termination,
...
one. Local code cannot prevent attempts on other hosts, so independent
public-ledger review is part of the acceptance gate.

**No seed selection by non-execution.** If the publicly frozen window closes
without a registered attempt, publish its status as **ABANDONED** with its
freeze hash, selected round, window, beacon availability and reason; the
one-attempt allowance is consumed. An unavailable beacon is documented as
ABANDONED when the window closes, not exchanged for another round. Executing
the confirmatory fixtures from the published seed outside the registered
attempt, including a private dry run before registration, violates this
protocol and invalidates the confirmatory claim if discovered.

Any later study of the same performance claim must disclose **every** earlier
frozen window, in order, with its public evidence and final status:
ABANDONED, INVALID, PASS or FAIL. Its registration must explain how selection
from the complete history is handled; a new freeze and beacon do not erase an
earlier abandoned or failed draw. Under this one-shot acceptance rule, a later
run cannot be substituted for the spent attempt or treated as an independent
first confirmatory result for the same claim. A prospective multi-study rule
would require separate preregistration and independent approval **before**
seeing any new seed.
...
- **ABANDONED:** frozen window closed without a registered attempt; publish its
  disposition and include it in every later study's complete history. It is
  neither a performance PASS nor a performance FAIL.

...
PROTOCOL = "PYTHAGOREAN_FILTER_CONFIRMATORY_SPEC_v0_3.md"
BITS = 100_000
N_PAIRS = 30
REPEATS = 8
...
def validate_gate(record_path):
    ...
    if len(randomness) != 32 or not signature:
    ...
    if attested < observed or attested > now or attested.date() != now.date():
        raise SystemExit("idle attestation must follow beacon and be current UTC date")
    seed_material = (b"pythagorean-filter-v0.3\x00" + bytes.fromhex(script_hash)
...
def main():
    ...
    if args.smoke:
        ...
        smoke_seed = "smoke-only-separate-stream-v0.3"
        seen = set()
